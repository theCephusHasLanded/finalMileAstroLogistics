const { getAPlanet } = require("../queries/planets.js");
...
review.get("/planet/:planetId", async (req, res) => {
  const { planetId } = req.params;
  try{
    const allReviews = await getAllReviews(planetId);
    res.json(allReviews)
  } catch (err) {
    res.json(err)
  }
...
//CREATE
review.post("/add/planet/:planetId", async (req, res) => {
